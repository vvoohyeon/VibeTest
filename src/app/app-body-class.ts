 ...
 * blue `rgb(52 119 255 / 14%)` — 를 `var(--canvas)` 위에 깔았다. 어느 쪽도 이 시스템의 색이 아니고,
 ...
 *
 * wash를 걷어내면 바닥은 `--canvas` 하나가 된다. theme cut은 이 파일이 읽던 `--bg`를 그
 * 역할로 매핑했고, 5c 묶음 C가 그 한 겹의 이름을 걷어내 여기서 역할을 직접 부른다.
 *
...
export const APP_BODY_CLASSNAME =
  'bg-[var(--canvas)] min-h-screen text-[var(--ink)] leading-[1.5] [font-family:var(--font-sans)]';
