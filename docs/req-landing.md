...
#### Instruction 표시 Variant 분류

모든 instruction 표시는 아래 두 variant 중 정확히 하나로 분류된다.

| Variant | 트리거 조건 | CTA |
|---|---|---|
| **Consent-branch** | Landing ingress + Consent UI로 동의 여부를 아직 선택하지 않은 사용자 | [Accept All and Start] / [Deny and Abandon] |
| **Default** | 그 외 모든 경우 (consent 선택 완료, 딥링크 유입 등) | [Start] |

**Consent-branch 판정 기준 불변식**:
- 트리거는 반드시 **Landing ingress이면서 consent 미선택** 두 조건을 동시에 만족해야 한다.
- 딥링크·직접 URL 유입은 consent 미선택이더라도 Consent-branch를 표시하지 않는다.
- consent 선택이 이미 완료된 사용자(Agree All 또는 Disagree All)는 Landing ingress이더라도
  Default variant를 표시한다.

#### Consent-branch Variant 계약

**표시 내용**: 디폴트 instruction 메시지 + opt_in 유도 메시지.

**[Accept All and Start]**:
- Agree All을 영구 저장한다 (Consent UI에서 Agree All 선택과 동일 효과).
- 화면 하단의 consent banner를 즉시 dismiss한다.
- runtime entry commit을 실행한다 (Test Flow Requirements §3.4, §3.6 준용).
- instructionSeen을 기록한다 (Start 클릭 직후 시점 준용).

**[Deny and Abandon]**:
- Disagree All을 영구 저장한다.
- runtime entry commit 없이 랜딩으로 복귀한다.
- pre-test abandonment context로 취급한다 (§3.6 instruction 이탈 계약 준용).
- consent 저장과 랜딩 복귀는 원자적으로 실행한다.

#### Default Variant — 딥링크 + consent 미선택 특수 케이스

딥링크 유입 + consent 미선택 사용자는 Default variant instruction과 함께 하단 Consent UI가 동시에 노출된다.

**[Start] 클릭 시 (consent 미선택 상태)**:
- 진행 불가 안내 팝업을 표시한다.
  - 내용: "하단의 쿠키 사용 여부를 동의한 다음 진행할 수 있습니다."
  - CTA: [Close] 1개.
  - [Close] 클릭 시: 팝업 닫힘. instruction과 하단 Consent UI 유지.
- runtime entry commit을 실행하지 않는다.

**하단 Consent UI Agree All 선택 시**:
- Agree All 영구 저장. banner dismiss.
- instruction이 [Start] 활성 상태로 전환된다. 이후 [Start] 클릭은 정상 진행.

**하단 Consent UI Deny 선택 시 — 진입 카드 타입별 분기**:

| 진입 카드 타입 | Deny 선택 시 동작 |
|---|---|
| `available` | 확인 팝업 표시: "동의하지 않으면 이 테스트를 진행할 수 없습니다. 그래도 거부하시겠습니까?" / [확인] [취소]. [확인] 선택 시: Disagree All 영구 저장 + 랜딩 복귀. [취소] 선택 시: 팝업 닫힘, instruction + Consent UI 유지. |
| `opt_out` | 확인 팝업 없이 Disagree All 영구 저장. instruction에서 [Start] 활성 상태 유지 (opt_out 카드는 Disagree All 상태에서도 진입 가능). |

**[딥링크 + available 카드 + Deny 최종 확정 후] 랜딩 복귀 계약**:
- Disagree All 저장 완료 후 랜딩으로 복귀한다.
- 복귀 시 available 카드는 비노출 상태이며 opt_out 카드만 카탈로그에 표시된다.

#### OPTED_OUT 상태에서 available 카드 직접 URL 접근

이미 Disagree All이 영구 저장된 사용자(OPTED_OUT)가 available 테스트에 직접 URL로 접근하면 즉시 랜딩으로 복귀한다.
복귀 시 available 카드는 비노출 상태이며 opt_out 카드만 카탈로그에 표시된다.
별도 안내 팝업·panel 없이 즉시 복귀한다. instruction overlay는 표시하지 않는다.
opt_out 카드에 OPTED_OUT 상태에서 직접 URL로 접근하면 정상 진입한다. §13.9 계약을 따른다.
...
**Verification**:
1. Automated: Landing ingress + 미선택 → Consent-branch variant 표시를 검증한다.
2. Automated: 딥링크 + 미선택 → Default variant + 하단 Consent UI 동시 노출을 검증한다.
3. Automated: [Accept All and Start] → Agree All 영구 저장 + banner dismiss + test start를 검증한다.
4. Automated: [Deny and Abandon] → Disagree All 영구 저장 + 랜딩 복귀를 검증한다.
5. Automated: 딥링크 + available + Deny 확정 → Disagree All 저장 + 랜딩 복귀를 검증한다.
6. Automated: 딥링크 + opt_out + Deny → 확인 팝업 0건 + [Start] 활성 유지를 검증한다.
7. Automated: [Start] 클릭 (consent 미선택) → 진행 불가 팝업 표시 + runtime entry commit 0건을 검증한다.
8. Automated: OPTED_OUT 상태에서 available 카드 직접 URL 접근 시 instruction 없이 즉시 랜딩 복귀, opt_out 카드만 카탈로그 표시를 검증한다.
...
- opt_out 카드 진입은 §13.5 consent 분기의 적용 대상이 아니다. consent 상태와 무관하게 분기 없이 바로 테스트를 시작한다.
...
19. Traceability Closure: 이 섹션(§14.2)의 모든 블로킹 항목이 최소 1개 이상의
    자동 단언과 매핑되어야 하며, 미매핑 `0건` PASS (Section 14.3).
20. **Instruction Consent-branch Display**: Landing ingress + consent 미선택 시 Consent-branch variant 표시. 딥링크 + 미선택 시 Default variant + 하단 Consent UI 동시 노출. consent 선택 완료 사용자에게 Consent-branch 표시 `0건` PASS.
21. **[Accept All and Start] Contract**: Agree All 영구 저장 + banner 즉시 dismiss + runtime entry commit 실행 + instructionSeen 기록 원자성 PASS.
22. **[Deny and Abandon] Contract**: Disagree All 영구 저장 + 랜딩 복귀 + runtime entry commit `0건` + instructionSeen 기록 `0건` PASS.
23. **Deep link Consent UI Deny Branch**: available 카드 유입 시 확인 팝업 표시 후 Deny 확정 → Disagree All 저장 + 랜딩 복귀. opt_out 카드 유입 시 확인 팝업 `0건` + [Start] 활성 유지. 딥링크 [Start] 클릭(미선택) → 진행 불가 팝업 + entry commit `0건` PASS. OPTED_OUT 상태에서 available 카드 직접 URL 접근 시 instruction 없이 즉시 랜딩 복귀, opt_out 카드만 카탈로그 표시 PASS (§13.5).
