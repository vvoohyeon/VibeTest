...
- `unavailable` 판정의 단일 소스는 fixture(또는 `variant-registry.generated.json`) 파일의 `attribute` 필드다. `unavailable: boolean` 레거시 필드는 `attribute: 'unavailable'`으로 해석한다 (req-test.md §2.5 레거시 호환 기준).
...
- 현재 fixture 경로에서 `attribute` 5종을 즉시 구현한다. registry phase 이후로 미루지 않는다.
