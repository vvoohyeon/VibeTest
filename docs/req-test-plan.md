...
> Test Flow Phase 1 이상은 variant registry에서 `attribute` 컬럼을 읽는 것을 전제하므로, 카드 타입 시스템 구현이 선행되지 않으면 Phase 2 registry 인터페이스 설계가 불완전해진다.
...
| 산출물 | 내용 | 참조 |
|---|---|---|
| `attribute` 컬럼 인식 | `source-fixture.ts` 및 registry builder/resolver가 `attribute` 5종 값을 인식하도록 갱신 | req-landing.md §2 |
| `loadVariantRegistry()` / `resolveLandingCatalog()` 갱신 | `hide`, `debug` 카드를 end-user 카탈로그에서 제외하고, `attribute` 5종을 현재 fixture 경로에 즉시 반영 | req-landing.md §2, §13.9 |
...
| `unavailable: boolean` 레거시 필드 호환 | `true` → `attribute: 'unavailable'`으로 해석 | req-test.md §2.5 |
| 관련 unit/e2e/QA 스크립트 갱신 | `check-phase5-card-contracts.mjs`, `landing-card-contract.test.ts`, `grid-smoke.spec.ts` 등 5종 카드 타입 계약 반영 | — |

### 완료 조건

- [x] `loadVariantRegistry()`와 `resolveLandingCatalog()`가 `hide`, `debug` 카드를 end-user 카탈로그에서 제외함
- [x] Disagree All 상태에서 `available` 카드 0건, `opt_out` 카드 정상 노출을 자동으로 검증하는 단언이 존재함
- [x] `docs/req-test.md` §2.5 `attribute` 계약과 코드베이스 구현이 정합함
...
- [x] landing ingress + `OPTED_IN` + `available` 경로에서 variant별 instruction 본문 + [Start]만 표시되고, runtime이 landing pre-answered `scoring1` 이후 `q.1`(존재 시) 또는 `scoring2`부터 진행함을 자동 검증하는 단언이 존재함 (`tests/e2e/transition-telemetry-smoke.spec.ts` blocker 6/28)
- [x] landing ingress + `OPTED_OUT` + `opt_out` 경로에서 variant별 instruction 본문 + [Start]만 표시되고, runtime이 landing pre-answered `scoring1` 이후 `q.1`(존재 시) 또는 `scoring2`부터 진행함을 직접 검증하는 자동 단언이 존재함 (`tests/e2e/transition-telemetry-smoke.spec.ts` blocker 6)
...
- [x] 딥링크 유입 + `OPTED_IN` + `available` 경로에서 plain instruction + [Start]만 표시되고, runtime이 `q.1`(존재 시) 또는 `scoring1`부터 진행함을 자동 검증하는 단언이 존재함 (`tests/e2e/consent-smoke.spec.ts`, `tests/e2e/transition-telemetry-smoke.spec.ts` blocker 28)
- [x] 딥링크 유입 + `OPTED_OUT` + `opt_out` 경로에서 [Start] 클릭 후 runtime이 `q.1`(존재 시) 또는 `scoring1`부터 진행함을 직접 검증하는 자동 단언이 존재함 (`tests/e2e/consent-smoke.spec.ts`)
...
- `question-bank.ts`의 unknown variant generic fallback은 이미 제거되었다. 현재 route bootstrap은 `resolveLandingTestCardByVariant()` miss 시 `notFound()`로 fail-close 하며, same-route recoverable invalid-variant recovery page 연결만 미구현 상태로 남아 있다.
...
Question = {
  index: QuestionIndex        // canonical 1-based, 불변
  poleA: string
  poleB: string
  questionType: QuestionType  // 'scoring' | 'profile' (sync parser가 생성)
...
VariantSchema = {
  ...
  questions: Question[]           // scoring + profile 문항 모두 포함. canonical 실행 순서 소스
}
...
- `Question.index`는 canonical 1-based 인덱스이며, sync normalization 이후 유일해야 한다.
- `questionType`는 source-authored column이 아니라 sync parser 산출물로 취급한다.
...
3. 각 question의 `index`는 canonical 1-based이며, sync normalization 이후 동일 variant 내 중복이 없어야 한다.
...
> canonical index uniqueness는 sync parser 보장 + question model validator 재검증의 이중 방어로 유지한다.
...
- Questions sync parser는 `q.*` row가 source row 순서에서 contiguous인지 검증해야 한다. 중간에 scoring row가 끼어드는 구조는 차단한다.
- Questions sync parser는 source row 순서를 유지한 canonical index 재번호 후 중복 index가 없음을 보장해야 한다.
- 각 variant에는 최소 1개의 scoring question, 즉 `scoring1`이 반드시 존재해야 한다.
- landing preview derivation은 `scoring1`을 대상으로 하므로, preview target이 없는 variant는 차단한다.
...
**Questions sync normalization 검증**:
- source row `seq` 파싱 규칙:
  - `q.{n}` → profile
  - `{n}` 숫자만 → scoring
- `q.*`는 source row 순서 기준으로 ordering-preserved 상태를 유지해야 하며, profile block이 끊긴 뒤 다시 `q.*`가 등장하면 차단한다.
- canonical `questions[]`는 source row 순서를 유지한 1-based 재번호 결과여야 한다.
- `scoring1`은 preview derivation과 landing pre-answer의 공통 기준이므로 반드시 존재해야 한다.
...
- canonical index와 user-facing scoring label을 동일시하지 않는다. derivation은 canonical `questions[]`에서 scoring subset만 필터링해 수행한다.
...
**EGTT 예시 (axisCount=1)**:
```typescript
// scoring 문항만 필터링 후 처리 (canonical index 1의 q.1 profile 문항은 제외될 수 있음)
...
| Questions sync parser — `q.*` contiguous / ordering-preserved | pass | #7 (간접) |
| Questions sync parser — profile block 재등장 | blocking error | #7 (간접) |
| Questions sync parser — canonical index 재번호 후 유일성 | pass | #7 |
| Questions sync parser — `scoring1` 부재 | blocking error | #30 |
| preview derivation — first row가 profile이고 first scoring이 별도일 때 | preview가 `scoring1`에서 파생됨 | #30 |
...
- `src/app/[locale]/test/[variant]/page.tsx`는 `resolveLandingTestCardByVariant(locale, variant)`를
  호출하고, fixture lookup miss 시 즉시 `notFound()`를 반환한다.
- `question-bank.ts`는 `variant` 문자열을 직접 받지 않고, resolve된 `LandingTestCard`만 받아
  landing preview / first scoring payload를 구성한다. unknown variant를 generic 문항으로 대체하는 fallback 경로는 존재하지 않는다.
...
현재 동작은 `resolveLandingTestCardByVariant()` miss → `notFound()` hard fail이며,
