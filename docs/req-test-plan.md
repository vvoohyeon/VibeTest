...
>
> - **Source topology·Questions 구조·row classification·canonical normalization·numbering 분리·landing preview 파생·scoringLogicType canonical location** → `requirements.md §1 Confirmed policy alignment` 및 `req-test.md §1.3`
...
**핵심 결정 (변경 시 새 ADR 필요)**:
- `src/features/test` 디렉토리가 canonical test destination surface. `src/features/landing/test/*` 파일 제거 완료, shim/alias 없음.
- `src/features/test/domain/index.ts`는 Phase 1 순수 도메인의 유일한 public surface.
- `VariantId`: `string & { readonly __brand: 'VariantId' }` 고정. object-wrapping 금지.
- `QuestionIndex`: `number & { readonly __brand: 'QuestionIndex' }` 고정.
- `validateVariant()` 시그니처와 `{ ok: false; reason: 'MISSING' | 'UNKNOWN' | 'UNAVAILABLE' }` union shape 동결. shape 변경은 새 ADR 대상.
- **live route/runtime은 `src/features/test/domain/`을 아직 import하지 않는다.** live wiring은 Phase 4+ 소유.
- invalid-variant 처리는 현재 `resolveLandingTestCardByVariant()` miss → `notFound()` hard fail. Phase 4 소유: `validateVariant()` ok:false → §6.1 에러 복구 페이지 wiring.
...
- 런타임 key migration은 Phase 3에서 수행. 현재 live key는 `project-analysis.md §6` 기준.
...
**핵심 결정**:
- representative anchor SSOT: `tests/e2e/helpers/landing-fixture.ts` (`PRIMARY_AVAILABLE_TEST_VARIANT=qmbti`, `PRIMARY_OPT_OUT_TEST_VARIANT=energy-check`).
...
| `computeScoreStats()`, `deriveDerivedType()` | `derivation.ts` | schema-driven 도출. profile 문항 제외, MBTI 하드코딩 없음. `axisMatchesQuestion()` direct helper export (index.ts public surface 미포함) |
...
- `scoring` question에서만 필수, `profile` question에서는 optional. EGTT `q.*` profile row의 source shape(`poleA`/`poleB` 없음)와의 충돌 해소.
- 변경 범위: `types.ts`, `validate-question-model.ts`, `question-source-parser.ts`, 관련 unit tests.

> **동결 계약 (TypeScript 시그니처 전체)**: `project-analysis.md §5.5.1` 참조.  
> shape·enum 값 변경은 새 ADR 필수.

**Live runtime 현황 (Phase 4+ 소유)**:
- `test-question-client.tsx`와 page.tsx는 `src/features/test/domain/`을 아직 import하지 않는다 (compatibility shell).
- Phase 4 첫 커밋 소유: `validateVariant()` ok:false → §6.1 에러 복구 페이지 wiring.
...
아래 세 ADR은 모두 확정 완료 상태다. Phase 2 구현 착수 조건 충족.
...
| `loadVariantRegistry()` | 환경별 registry 파일 로딩 인터페이스. production과 dev fixture가 동일 시그니처. ADR-D 확정(versioned file)에 따라 현재 구현은 cached fixture-built registry를 반환하며, generated production 분기는 Sync GitHub Action 구현 후 연결한다 |
...
- [x] Phase 11 executable-only closure 기준으로 future blocker entry를
  재분류할 타이밍과 책임자 기록 완료.
  **현재 상태(2026-04-21)**: blocker 20~23은
  `tests/e2e/consent-smoke.spec.ts` automated assertion으로 전환됐다.
  blocker 24, 25, 26, 27, 28, 30에는 여전히 manual checkpoint가 남아 있고,
  blocker 27과 28은 추가 automated evidence도 함께 가진 mixed-evidence 상태다.
  남은 manual checkpoint의 재분류 책임은 해당 Phase 첫 커밋 작성자가 진다.
...
- 미완료 시: Phase 11 DoD의 "§9.1 hook 6개 위치 확보 누락 0건" 항목을 자동으로 단언할 수 없음. blocker #18은 현재 `final_submit` payload/validation 자동 evidence를 갖지만, §9.1 hook 6개 전체 closure evidence는 아직 미완료 상태로 남는다.
