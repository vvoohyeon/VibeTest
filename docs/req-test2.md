...
3. **데이터 소스 동기화 계약**
   - Google Sheets 연동 구조 및 3-Sheet 구성 계약 (§12.2)
   - variant-registry 생성 파이프라인 — GitHub Action (§12.3)
   - Runtime lazy validation 계약 — D-3 Per-request (§12.5)
   - cross-sheet 불일치 처리 — C-3: Action 블로킹 + Runtime variant-scoped fallback (§12.5)
   - fixture 역할 재정의 — dev/test fallback으로 한정 (§12.3)

### 1.2 Non-goals

- 랜딩 카탈로그 UI 및 카드 인터랙션 세부 규칙
- blog 진입/표시 규칙
- admin analytics / ops 콘솔 · Sheets 동기화 운영 대시보드 (sync 계약 자체는 §12 포함)
...
| 데이터 소스 | Google Sheets (3-Sheet 구성: Questions / Results / Landing Card Metadata). fixture는 dev/test fallback으로 유지, production 소스 금지 |
| variant-registry 생성 방식 | GitHub Action (main branch push 트리거) → 3-Sheet cross-sheet 검증 → `variant-registry.generated.json` 커밋 → Vercel 배포 트리거. 검증 실패 시 커밋 없음 (last-known-good 유지) |
| Runtime 초기화 방식 | D-3 Per-request Lazy: variant 첫 진입 시 `validateVariantDataIntegrity()` 실행, 결과 variant별 in-memory 캐싱 |
| Landing `unavailable` 표시 기준 | Landing Card Metadata Sheet `unavailable` 컬럼 오퍼레이터 직접 기입. 시스템이 자동으로 덮어쓰지 않음 |
| Cross-sheet 불일치 처리 | C-3: Action 블로킹(1차 방어선) + Runtime variant-scoped fallback(2차 방어선). partial activation 금지 |
| Action 실패 알림 | 비포함. Action 로그 확인은 오퍼레이터 책임 |
...
| Sheets sync / variant-registry 구조 변경 | 12.2, 12.3, 12.4, 11.2 |
| Cross-sheet 검증 정책 변경 (Action 블로킹 또는 Runtime fallback 조건) | 12.5, 11.2 |
| Runtime lazy validation 정책 변경 (D-3 캐싱 범위 포함) | 12.5, 2.2, 5.1, 11.2 |
| `unavailable` 컬럼 정책 변경 (오퍼레이터 기입 vs 시스템 자동) | 12.4, 12.5, 11.2 |
| in-progress session 비파괴 계약 변경 | 12.6, 2.6, 7.3, 11.2 |
| 데이터 로딩 실패 처리 정책 변경 | 12.7, 5.1, 11.2 |
| fixture 역할 변경 (fallback 범위 조정 또는 production 허용 여부 변경) | 12.3, 12.7, 11.2 |
...
29. **Variant-registry 존재 및 파싱**: `variant-registry.generated.json`이 미존재하거나 JSON 파싱 실패 시 runtime이 crash하지 않는다. "data unavailable" 상태로 처리되어 §5.1 에러 복구 페이지로 이동한다. fixture fallback이 동일 인터페이스를 통해 dev/test 환경에서 정상 동작함을 검증한다.
30. **Runtime Lazy Validation (D-3)**: 각 variant의 첫 진입 시 `validateVariantDataIntegrity()` 가 실행된다. 검증 실패 variant는 진입이 차단되고 §5.1 에러 복구 페이지로 이동한다. 동일 variant 재진입 시 캐싱된 결과를 사용한다 (재실행 없음). 검증 실패 variant가 차단되는 동안 다른 variant는 정상 서비스된다.
31. **Landing `unavailable` 오퍼레이터 기입 반영**: Landing 카탈로그가 `variant-registry`의 `unavailable` 필드를 그대로 반영한다. 시스템이 자동으로 `unavailable`을 덮어쓰는 경우 `0건`. `unavailable = false`로 기입된 variant라도 runtime 검증 실패 시 진입이 차단된다 (두 레이어 독립성 확인).
32. **in-progress Session 비파괴**: registry 갱신 또는 재배포 후에도 기존 세션의 응답 데이터(localStorage)가 유지된다. registry 갱신이 storage를 직접 조작하는 경우 `0건`. blocker 1~32 모두 최소 1개 자동 단언 매핑.
...
## 12. Data Source & Sync Contract

### 12.1 개요 및 원칙

본 섹션은 FRS REQ-F-020 / REQ-F-026에서 요구하는 Google Sheets 연동 계약을 Test Flow 1급 계약으로 격상한 내용이다.  
이 계약을 위반하는 구현은 §11.2 릴리스 블로커 #29–32에 의해 차단된다.

**절대 원칙**:
- **partial activation 금지**: 스키마 검증을 통과한 variant 전체만 활성화. 일부만 활성화 불가.
- **last-known-good 유지**: Action 검증 실패 시 기존 `variant-registry.generated.json`을 덮어쓰지 않음.
- **in-progress session 비파괴**: sync 또는 재배포로 인한 registry 갱신이 진행 중 세션의 응답 데이터를 파괴하지 않음.

---

### 12.2 Sheet 구성

| Sheet | 역할 | variant 연결 기준 |
|---|---|---|
| **Questions** | 모든 variant의 질문 데이터 | `variantId` 컬럼 |
| **Results** | 모든 variant의 결과 콘텐츠 데이터 | `variantId` 컬럼 |
| **Landing Card Metadata** | 랜딩 카드 / 블로그 카드 메타데이터 | `variantId` 컬럼 |

- 각 Sheet는 독립적 단일 소스. Sheet 간 cross-reference는 `variantId`로만 수행한다.
- 언어 컬럼 누락 시 default locale fallback 의무 (FRS REQ-F-026). 차단 조건이 아니다.
...
```
GitHub Action (main branch push 트리거)
  → 3개 Sheet 읽기
  → cross-sheet 검증 (§12.5 1차 방어선)
  → 검증 통과 시: variant-registry.generated.json 커밋
  → Vercel 배포 트리거
  → 검증 실패 시: 커밋 없음, 기존 파일 유지 (last-known-good)
```
...
| 환경 | 데이터 소스 |
|---|---|
| Production | `variant-registry.generated.json` (GitHub Action 생성) |
| Local Dev / Test | fixture 파일 (동일 경로, 동일 인터페이스) |

- 런타임 코드는 환경에 따라 분기하지 않는다. 경로와 인터페이스가 동일하므로 파일 교체만으로 전환.
- fixture는 dev/test fallback 역할만 수행한다. production 소스로 사용 불가.

#### Action 실패 처리

- 검증 실패 시 파일 커밋 없음. 기존 `variant-registry.generated.json` 유지.
- 알림 메커니즘은 이 계약에 포함하지 않는다. Action 로그 확인은 오퍼레이터 책임.

---

### 12.4 variant-registry 파일 구조

`variant-registry.generated.json`은 아래 최상위 구조를 가진다.
...
- `unavailable` 필드: Landing Card Metadata Sheet의 오퍼레이터 기입값을 그대로 반영한다. 시스템이 자동으로 덮어쓰지 않는다.
- 파일 미존재 또는 JSON 파싱 실패 시: runtime "data unavailable" 처리 (§12.7 참조).

---

### 12.5 Cross-sheet 검증 계약 (C-3)

#### 1차 방어선 — Action 검증 (배포 시점)

- Questions / Results / Landing Card Metadata 3개 Sheet 전체 cross-sheet 검증.
- 불일치 1건이라도 존재하면 파일 커밋 없음. 기존 파일 유지.
- 검증 항목:
  - Landing Metadata에 존재하는 모든 `variantId`가 Questions에도 존재해야 함.
  - Questions에 존재하는 모든 `variantId`가 Results에도 존재해야 함.
  - 각 variant의 question set이 `validateVariantDataIntegrity()` 조건을 통과해야 함 (§5.2).
  - 언어 컬럼 누락 시 default locale fallback 처리 — 오류로 처리하지 않음.

#### 2차 방어선 — Runtime 검증 (variant 진입 시점, D-3 Lazy)

- 각 variant가 처음 요청될 때 해당 variant에 대한 `validateVariantDataIntegrity()` 실행.
- 결과는 variant별로 in-memory 캐싱. 이후 동일 variant 요청은 캐싱된 결과 사용.
- 검증 실패 시: 즉시 §5.1 에러 복구 페이지 이동. 다른 variant 정상 서비스 유지.
- `unavailable` 판정은 시스템이 수행하지 않는다. Landing Card Metadata Sheet의 오퍼레이터 기입값을 신뢰.

#### 두 방어선의 관계

- 독립적이며 additive. Metadata `unavailable = false`이어도 runtime 검증 실패 시 진입 차단.
- Landing 카탈로그의 `unavailable` 표시 = 오퍼레이터 의도적 숨김.
- Runtime 검증 실패 = 콘텐츠 무결성 안전망. 별도 역할.

---

### 12.6 in-progress Session 비파괴 계약

- sync 또는 재배포로 인해 `variant-registry.generated.json`이 갱신되어도, 이미 진행 중인 세션의 응답 데이터(localStorage)는 파괴되지 않는다.
- session storage와 variant-registry는 독립적으로 관리된다. registry 갱신은 storage를 직접 조작하지 않는다.
- 진행 중 세션에서 사용 중인 variant의 schema가 갱신된 경우: 다음 진입 시 §2.2 variant resolution contract 및 §12.5 runtime 검증을 통해 처리.
...
| 상황 | 처리 |
|---|---|
| `variant-registry.generated.json` 미존재 | runtime "data unavailable" → §5.1 에러 복구 페이지 |
| JSON 파싱 실패 | 동일 |
| runtime cross-sheet 검증 실패 (variant 단위) | 해당 variant 진입 차단 → §5.1 에러 복구 페이지. 나머지 variant 정상 서비스 |
| 언어 컬럼 누락 | default locale fallback. 차단하지 않음 |

fixture는 dev/test 환경에서만 사용. fixture 자체는 fail-safe. production에서 fixture 미사용.
