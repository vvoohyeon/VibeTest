...
> **구성**: Phase 전체 요약 → Phase 1 세부 계획 → Phase 2 예비 요약 → Phase 3 예비 요약  
...
| **2** | Data Sync Foundation | variant-registry 생성 스크립트, cross-sheet 검증, runtime lazy validator, fixture fallback 인터페이스 | 1 |
...
| **7** | Derivation · Loading Screen | scoreStats/derivedType 계산, 5초 최소 로딩 AND 조건, back-from-loading | 1, 2, 6 |
...
| **10** | Error States · Terminal Exclusivity · Cleanup Set | commit-failure / derivation-failure taxonomy, §7.1 전이 테이블, §7.3 cleanup 원자성 | 4, 5, 6, 7 |
| **11** | Telemetry Skeleton · Release Gate | §8.1 hook 6개, traceability closure (blocker 1~32 매핑) | 전체 |
...
## Part 3 — Phase 2 예비 요약 (Data Sync Foundation)

> Phase 1 완료 전에 이 섹션을 확인한다. Phase 2는 모든 runtime 로직이 데이터를 신뢰하기 전에 확립해야 하는 "데이터 소스 계약 레이어"다.

### 목적

Phase 2는 아래 세 관심사를 **하나의 레이어**에서 확립한다. 이 셋을 Phase 3 이후로 미루면, Phase 3부터의 경로 분기 로직이 "데이터가 항상 존재한다"는 암묵적 전제에 의존하게 되어 sync 계약 위반 감지가 불가능해진다.

1. **variant-registry 생성 파이프라인** (§12.3): GitHub Action이 3개 Sheet를 읽고 cross-sheet 검증 후 `variant-registry.generated.json`을 커밋.
2. **Runtime lazy validation 인터페이스** (§12.5): variant 첫 진입 시 `validateVariantDataIntegrity()` 실행, 결과 in-memory 캐싱.
3. **환경별 fixture fallback 인터페이스** (§12.3, §12.7): Local Dev/Test에서 fixture가 동일 경로·인터페이스로 registry를 대체.

### 핵심 설계 결정 (Phase 2 착수 전 확정 필요)

**variant-registry 파일 포맷**: `variant-registry.generated.json`의 top-level 구조 및 variant 항목 구조를 Phase 2 착수 전 §12.4에서 확정해야 Phase 3 이후 모든 data accessor가 동일 구조를 참조한다.

**runtime lazy cache 저장소**: in-memory Map (variant별 캐싱). 서버 재시작 시 리셋되나 per-request 재실행보다 낮은 오버헤드. Phase 2에서 이 캐시 인터페이스를 정의하지 않으면 Phase 4 이후 variant 진입 경로가 직접 파일을 읽는 분산 패턴이 생긴다.

### 주요 산출물 (예비)

| 산출물 | 내용 |
|---|---|
| `sync_script.js` (또는 `scripts/sync-sheets.ts`) | 3개 Sheet 읽기 → cross-sheet 검증 → `variant-registry.generated.json` 생성 |
| `.github/workflows/sync-sheets.yml` | main branch push 트리거, 검증 실패 시 커밋 없음 |
| `loadVariantRegistry()` | registry 파일 읽기 + JSON 파싱. 실패 시 "data unavailable" 상태 반환 |
| `getVariantLazyValidator(variantId)` | variant별 캐싱 포함 lazy validation 인터페이스. 첫 호출 시 `validateVariantDataIntegrity()` 실행, 이후 캐시 반환 |
| fixture fallback 인터페이스 | dev/test 환경에서 동일 경로·인터페이스로 registry를 대체하는 fixture 파일 |

### Phase 2가 Phase 1에 주는 설계 제약

Phase 1의 `validateVariantDataIntegrity()`가 Phase 2 lazy validator의 핵심 실행 대상이다. Phase 1에서 이 함수가 `VariantSchema` 단위로 독립 실행 가능한 pure 함수로 구현되어야 Phase 2에서 per-variant 캐싱 패턴이 가능하다.

### Phase 2 완료 후 Phase 3이 안전하게 전제할 수 있는 것

- `loadVariantRegistry()`가 항상 정형화된 결과(성공 | data-unavailable)를 반환함
- 등록된 variant 목록을 신뢰할 수 있는 소스에서 읽을 수 있음 (`validateVariant`의 `registeredVariants` 인자 소스)
- variant 첫 진입 시 `validateVariantDataIntegrity()` 결과를 lazy cache에서 얻을 수 있음
- fixture와 generated registry가 동일 인터페이스를 공유함을 타입 수준에서 보장됨

---

## Part 4 — Phase 3 예비 요약

> Phase 1 완료 전에 이 섹션을 확인한다. Phase 3는 Phase 1의 순수 함수 위에 "상태가 저장되는 레이어"를 쌓는 첫 번째 Phase다.
...
**storage 추상 레이어 선택**:
Phase 3는 storage key 네이밍, store 구조, key 목록을 별도 구현/설계 문서에서 정의하는 것이 요구사항의 원칙 (§5.8, §7.3 구현 세부사항 위임 구절). Phase 3 착수 시 이 설계 문서 또는 ADR을 먼저 작성 권장.

**variant-scoped 격리**:
모든 storage 조작은 해당 variant 범위에만 영향을 준다. 다른 variant 데이터를 건드리지 않는다 (§5.8 삭제 원칙). Phase 3에서 이 격리 경계를 명확히 구조화하지 않으면 Phase 10의 cleanup set 원자성 검증이 불가능해진다.
