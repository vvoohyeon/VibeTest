...
## 2026-09-10 — blocker traceability 는 항목 단위로 남기고, 하드코딩된 항목 수만 제거한다

**배경.** `req-landing.md` §14.2 항목 4 의 「폭 변경 시 강제 종료 후 재계산」을 청구하는 단언이 하나도 없는데 traceability 검사는 초록이었다. 원인은 §14.3 의 닫힘 단위가 **항목**이고 §14.2 의 한 항목이 계약 열 개 이상을 쉼표로 묶기 때문이다(약 167 개 하위 계약 / 30 항목 / 단언 67). 부수로, 가드의 `blocker <= 30` 은 손으로 적은 상한이라 §14.2 에 항목이 하나 늘면 그 항목은 실패가 아니라 **침묵**으로 빠진다(원장 `L02` 의 재현).

**검토한 대안.** (A) 하위 계약마다 ID(`B4.6` 꼴)를 주고 「청구되지 않은 절 0 · 실재하지 않는 절을 청구하는 단언 0」의 양방향 불변식으로 절 단위 닫힘을 만든다. (B) 지금 비어 있는 이 계약 하나만 단언으로 채우고 등록부에 등재한다. (C) 가드가 항목 수를 §14.2 에서 읽게 해 하드코딩 상한을 없앤다. (D) 「현재 미매핑 N 건보다 늘면 실패」하는 래칫을 둔다.

**선택과 근거 — (B) + (C).** 사용자 지시가 이 둘로 범위를 확정했다. (B) 는 §14.2 항목 4 와 §6 Automated 5·11 이 이미 요구하는 계약을 실제로 실행하는 검사를 만든다 — 끝 상태만 보면 「먼저 재계산하고 나중에 닫는」 구현도 통과하므로 **순서**를 단언한다(새 plan 이 처음 관측되는 프레임에서 활성 Expanded 가 비어 있을 것). (C) 는 문제의 크기가 작고 기존 게이트가 그대로 검증하며, 양방향 대조(미등재·유령)와 「파싱 실패 자체가 실패」를 함께 넣어 조용히 무력해지는 경로를 막는다.

**기각한 대안과 이유.** (A) 는 **최종 목적지이지만 지금은 아니다** — 지금 한 번에 도입하면 미매핑 130여 건이 즉시 붉어져 릴리스가 막힌다. 절 ID 부여와 §15 예외 등재를 동반한 단계적 이행이 필요하며, 그것은 이 변경의 범위를 넘는다. (D) 는 **개체수에서 유도된 하한**이라 이 저장소가 이미 기각한 형태다(묶음 B 에서 미러 개체수 하한 `>80` 이 걸리자 상수를 낮추는 대신 불변식 둘로 옮긴 사례). 숫자로 관리하면 다음 세션이 숫자를 올려 통과시키는 길이 열리고, 무엇이 빠졌는지는 여전히 이름으로 남지 않는다. 예외는 세는 것이 아니라 §15 에 이름으로 등재한다.

**영향.** `tests/e2e/grid-smoke.spec.ts` 에 `assertion:B4-width-change-force-close` 가 생기고 `docs/blocker-traceability.json` 에 blocker 4 항목이 하나 늘었다(67 → 68). `scripts/qa/check-blocker-traceability.mjs` 의 `1..30` 루프가 §14.2 파싱 + 양방향 대조로 바뀌었다 — 정상 상태의 실패 집합은 변하지 않는다(blocker 5 `assertion:B5-overlay-focus` 는 BQ-07 이연분으로 그대로 붉다). §14.2·§14.3 본문은 고치지 않았다: 닫힘 단위가 항목이라는 규칙 자체는 (A) 가 도입될 때 바뀔 것이고, 그 전에 문구만 손대면 계약과 가드가 어긋난다. 남은 사각지대는 원장 `L18` 이 갖는다.
