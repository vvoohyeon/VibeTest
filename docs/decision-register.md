...
- **Implementation impact** — `docs/req-landing.md` §8.2 · §8.3 · §14.2 항목 13 개정(절 번호는 옮기지 않는다 — `tests/unit/contract-citations.test.ts` 가 `문서.md §N` 인용의 실재를 검사한다). `src/features/landing/grid/use-hover-intent-controller.ts` 에 `pointermove` 전용 seq 카운터와 스크롤 hold. 단계 계획은 `docs/plans/2026-09-10-ux-refactor-step1-scroll-hold-and-contract-tiers.md` 와 `-step2-`. **이 등재 시점에 코드 변경은 0 이다** — 결정이 구현에 앞선다
