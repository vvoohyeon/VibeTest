...
| 2026-09-07 | BQ-38 | 후속(5단계 Part 1) — 내비게이션 실현. 계층 없는 `a` 규칙을 `@layer base` 로(색 유틸리티가 앵커에서 조용히 지고 있었다), 테마 스와치 선택을 fill → 링, elevation 을 §6.7 방향으로, D-09 44px 바닥을 pill 클래스에, 오버레이 표면을 `--surface-raised`+`--border-strong` 으로. 드로어 닫기 컨트롤은 탭 순서 계약 때문에 유예 | BQ-38 후속 항목 · `docs/plans/2026-09-07-step5a-navigation-and-card-remainders.md` |
| 2026-09-07 | BQ-38 | 후속(5단계 Part 2) — 카드 잔여 정산. D-06 줄바꿈을 전 티어로, D-09 카드 닫기 44×44, D-07 `display` 스위치 제거로 페이드 복구(레이아웃 이동 23.33 → 0px). **D-05 마지막 1 건은 렌더 결과 기록이 틀려 고치지 않는다** — 두 그림자는 대체가 아니라 합쳐지고 픽셀 차이는 최대 3/255 | BQ-38 후속 항목 · `docs/plans/2026-09-07-step5a-navigation-and-card-remainders.md` |
